...
namespace Glueful\Extensions\StorageGcs\Tests\Unit\Schema;
...
final class SchemaManifestTest extends TestCase
{
    ...
    {
        $base = sys_get_temp_dir() . '/storage-gcs-mf-' . uniqid('', true);
        ...
        self::assertSame([], $manifest->migrationDescriptors()['glueful/storage-gcs']);
        self::assertNotContains('glueful/storage-gcs', $manifest->undeclaredGluefulPackages());
    }

    public function testResolvesWhenEnabledAloneAtItsDeclaredFloor(): void
    {
        $g = $this->manifest();
        $candidates = ['glueful/storage-gcs' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/storage-gcs',
            provider: $g['provider'],
